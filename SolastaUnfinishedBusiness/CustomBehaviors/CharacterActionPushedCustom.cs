...
public class CharacterActionPushedCustom : CharacterAction
{
    ...
    public override IEnumerator ExecuteImpl()
    {
        ...
        if (GameLocationCharacter.IsValidCharacter(ActionParams.ActingCharacter) &&
            !ActionParams.Positions.Empty())
        {
            var character = ActionParams.ActingCharacter;
            var position = ActionParams.Positions[0];
            if (character.RulesetActor.IsDeadOrDyingOrUnconscious)
            {
                yield break;
            }
            ...
            if (forceTurnTowardsSourceCharacterAfterPush && sourceCharacter != null)
            {
                ...
                var evt = GameLocationCharacterEventSystem.Event.RotationEnd;
                var characterTurnCoroutine =
                    Coroutine.StartCoroutine(character.EventSystem.UpdateMotionsAndWaitForEvent(evt));
                var sourceCharacterTurnCoroutine =
                    forceSourceCharacterTurnTowardsTargetAfterPush
                        ? Coroutine.StartCoroutine(sourceCharacter.EventSystem
                            .UpdateMotionsAndWaitForEvent(evt))
                        : null;
                while (!characterTurnCoroutine.Empty ||
                       (sourceCharacterTurnCoroutine != null && !sourceCharacterTurnCoroutine.Empty))
                {
                    if (sourceCharacterTurnCoroutine != null && !sourceCharacterTurnCoroutine.Empty)
                    {
                ...
                characterTurnCoroutine = null;
                sourceCharacterTurnCoroutine = null;
            }
