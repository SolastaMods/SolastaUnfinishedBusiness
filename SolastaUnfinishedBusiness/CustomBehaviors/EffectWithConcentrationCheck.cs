...
public class EffectWithConcentrationCheck
{
    private EffectWithConcentrationCheck()
    {
    }

    public static EffectWithConcentrationCheck Mark { get; } = new();


    internal static void ProcessConcentratedEffects(GameLocationCharacter locCharacter,
        int damage,
        string damageType,
        bool stillConscious)
    {
        ...
        var effects = rulesetCharacter.EnumerateActiveEffectsActivatedByMe()
            .Where(e => e.SourceDefinition.HasSubFeatureOfType<EffectWithConcentrationCheck>())
            .ToList();
    ...
    private static void RollConcentration(RulesetEffect effect, RulesetCharacter rulesetCharacter, int damage,
        string damageType,
        bool stillConscious)
    {
        ...
        if (stillConscious)
        {
            ...
            {
                rulesetCharacter.ConcentrationCheckRolled = (character, outcome, roll, rawRoll, modifier, dc, trends,
                    advantageTrends) =>
                {
                    character.LogConcentrationCheckRoll(effect, outcome, roll, rawRoll, modifier, dc, trends,
                        advantageTrends);
                };
